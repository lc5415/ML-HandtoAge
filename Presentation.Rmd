...
g1 = res %>% pivot_longer(-c(Epoch, StepSize), names_to = "Loss", values_to = "MAE") %>%
  ...
  facet_wrap(~StepSize)+theme_pubclean()+scale_color_brewer(palette = "Set1")+
...
g2 = res %>% group_by(StepSize) %>% summarise(MinTrain = min(Train.loss), MinTest = min(Test.loss)) %>%
  ...
  geom_point(aes(x = StepSize, y = Min.MAE, color = Loss))+scale_color_brewer(palette = "Set1")+
  theme_pubclean()
...
```{r}
g2+ggtitle("Minimum MAE against hyperparameter value choice")+theme_pubclean()+scale_color_brewer(palette = "Set1")
```
